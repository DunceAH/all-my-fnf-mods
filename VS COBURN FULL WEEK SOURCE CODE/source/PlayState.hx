...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'schoolEvil': //Week 6 - Thorns
				...
				if(!ClientPrefs.lowQuality) {
					...
					add(bg);
							...
							//case 'reef':
				  //  {
					//var emitter = new FlxEmitter(200, 100);
					//for (i in 0 ... 20)
				//	{
		...
		//	case 'underwater':
			//	   {
			//		var emitter = new FlxEmitter(504.45, -20);
				//	for (i in 0 ... 20)
				//	{
				//		var p = new FlxParticle();
        		//		p.makeGraphic(2, 2, 0xFFFFFFFF);
		...
		{
				case "coburn-water":
				FlxTween.tween(dad, {y: dad.y + 60}, 1.5, {ease: FlxEase.quadInOut, type: PINGPONG});

				case "bf-underwater":
				FlxTween.tween(dad, {y: dad.y + 300}, 1.5, {ease: FlxEase.quadInOut, type: PINGPONG});
		...
		{
				case "bf-water":
				FlxTween.tween(boyfriend, {y: boyfriend.y + 65}, 1.5, {ease: FlxEase.quadInOut, type: PINGPONG});
				
				case "coburn-underwater":
				FlxTween.tween(boyfriend, {y: boyfriend.y + 200}, 1.5, {ease: FlxEase.quadInOut, type: PINGPONG});
		...
		switch(curStage)
		{
			...
			case 'water':
				FlxTween.tween(dad, {y: dad.y + 50}, 1.5, {ease: FlxEase.quadInOut, type: PINGPONG});

			case 'reef':
			//credit to ohsat games they the bossss
					var OHSATGAMEPIC:FlxEmitter;

					OHSATGAMEPIC = new FlxEmitter(0, 0, 300);

					OHSATGAMEPIC.width = 5000;
					OHSATGAMEPIC.height = 3000;
					...
					for (i in 0...150)
						...
						OHSATGAMEPIC.launchMode = SQUARE;
						OHSATGAMEPIC.velocity.set(-10, 50, 10, 150);
						OHSATGAMEPIC.lifespan.set(0);		

						add(OHSATGAMEPIC);
						OHSATGAMEPIC.start(false, 0.09);
	...
	override public function update(elapsed:Float)
	{
		...
		//mess with
		if (gf.curCharacter == "gf-underwater"){
			gf.y += Math.sin(floatshit);
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'week1_nomiss' | 'week2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss' | 'coburn':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{
								...
								case 'coburn week':
									if(achievementName == 'coburn') unlock = true;
							}
						}
					case 'ur_bad':
